...
template<typename T> T poweri(T a,T b){T p=1;while(b>0){if(b&1){p*=a;}a*=a;b>>=1;}return p;} 
...
public:
	...
	SegTree(T n,T val){
		seg.clear();
		seg.resize(4*n+1,val);
	}
	void build(T ind, T low, T high, vector<T> &a, T f){
		if(low==high){
			seg[ind]=a[low];
			return;
		}
		T mid=(low+high)/2;
		build(2*ind,low,mid,a,f^1);
		build(2*ind+1,mid+1,high,a,f^1);
		if(f==1){
	...
	void point(T ind, T low, T high, T node, T val, T f){
		if(low==high){
			seg[ind]=val;
		}
		else{
			...
			if(node>=low && node<=mid){
				point(2*ind,low,mid,node,val,f^1);
			}
			else{
				point(2*ind+1,mid+1,high,node,val,f^1);
			}
			if(f==1){
				seg[ind]=(seg[2*ind]|seg[2*ind+1]);
			}
...
void solve(int tc)
{
	...
	SegTree<ll> S(m,0);
	S.build(1,1,m,a,(ll)n&1);
	ll tmp=1;
	while(q--){
		ll p,b;
		cin>>p>>b;
		S.point(1,1,m,p,b,(ll)n&1);
		cout<<S.seg[1]<<endl;
	}
}
int main(){
