...
void solve(int tc)
{
	...
	for(int i=1;i<=n;i++){
		...
		while(l<=h){
			ll mid=(l+h)/2;
			ll val=a[i]*mid-(b[i-1]-b[i-mid-1]);
			if(val<=k){
				// debug(i,a[i],mid,val);
				if(mid+1>ans.ss){
					ans.ss=mid+1;
	...
	//Editorial sol : psum + bs
	
	
	
	
	//https://codeforces.com/blog/entry/5486?#comment-338982 : 2pointers
	//https://codeforces.com/blog/entry/5486?#comment-366596 : 2pointers
}
int main(){
	start();
	time__("solve"){
