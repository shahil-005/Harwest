...
const long long N=(long long)(1e3+5);
...
void solve(int tc)
{
	//Without DSU : https://codeforces.com/contest/25/submission/117273213
	
	//using DSU
	int n;
	cin>>n;
	C.init(n);
	vector<pii> v1;	//remove
	vector<pii> v2;	//build new
	for(int i=1;i<=n-1;i++){
		int x,y;
		cin>>x>>y;
	...
	cout<<n1<<endl;
	for(int i=0;i<n1;i++){
		cout<<v1[i].ff<<" "<<v1[i].ss<<" "<<v2[i].ff<<" "<<v2[i].ss<<endl;
	}
}
int main(){
	start();
