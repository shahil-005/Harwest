// Author: shahil_005
// Time: 2021-05-13 18:00:04
// Problem: B - Kleene Inversion
// Contest: AtCoder - Japanese Student Championship 2019 Qualification
...
void solve(int tc)
{
	...
	for(int i=n;i>=1;i--){
		...
		ll y=ms.lower_bound(a[i]);
		
		ll t1=k*x;
		ll t2=(k*(k-1))/2;
		ll t3=modmul(y,t2,MOD);
		ll t4=modsum(t1,t3,MOD);
		// debug(t1,t2,t3,t4);
		// debug(i,x,y,t4,ans);
		
		ans=modsum(ans,t4,MOD);
		
		s.insert(a[i]);
	}
	cout<<ans;
	
	//Can be easily solved in o(n^2)
